...
def test_d4_1_perc_update():
    ...
    eq_(len(update),18)
    eq_(update[('real','trump')],1)
    eq_(update[('fake','son')],-1)
    eq_(update[('real',constants.OFFSET)],1)
    eq_(update[('fake',constants.OFFSET)],-1)
...
def test_d4_2a_perc_estimate():
    ...
    eq_(theta_perc[('fake','trump')],-1)
    eq_(theta_perc[('fake','clinton')],1)
    eq_(theta_perc[('real','claims')],-1)
    eq_(theta_perc[('real','about')],2)
    eq_(theta_perc_history[0][('real','about')],1)
...
def test_d4_2b_perc_accuracy():
    ...
    assert_greater_equal(evaluation.acc(y_hat_dv,y_dv),.7)
