...
def test_d1_2_agg():
    global x_dv
    eq_(counts_dv['donald'], 103)
    eq_(len(counts_dv), 1322)
    eq_(counts_dv['to'], 97)


def test_d1_3a_oov():
    global counts_tr, counts_dv
    eq_(len(preprocessing.compute_oov(counts_dv, counts_tr)), 322)
    eq_(len(preprocessing.compute_oov(counts_tr, counts_dv)), 3739)
...
def test_d1_4_prune():
    ...
    eq_(len(x_dv[95].keys()) - len(x_dv_pruned[95].keys()), 1)
